...
describe('JSSTREAM', () => {
  test('js read stream', () => new Promise((resolve, reject) => {
    als.scope()
    const id = randomBytes()
    als.set('id', id)
  ...
  test('js write stream', () => new Promise((resolve, reject) => {
    als.scope()
    const id = randomBytes()
    als.set('id', id)
...
describe('dns', () => {
  test('get addr info promise', async () => {
    als.scope()
    const id = randomBytes()
    const asyncId = als.currentId()
    als.set('id', id)
    await lookup('www.baidu.com')
      .then(() => {
        assert(als.get('id') === id)
        assert(als.currentId() !== asyncId)
      })
      .then(() => {
        topList.push(als.top())
        assert(als.get('id') === id)
...
describe('simple', () => {
  test('top Data', async () => {
    als.scope()
    als.set('name', 'main')
    assert(als.get('name') === 'main')
    await Promise.all([
      delay(200).then(() => {
        assert(als.get('name') === 'main')
        als.set('name', 'sleep1')
      }).then(() => {
        assert(als.get('name') === 'sleep1')
      }),
      delay(500).then(() => {
        assert(als.get('name') === 'main')
      })
...
describe('loop', () => {
  test('loopTestWhileWithOutScope', async () => {
    let count = 0
  ...
  test('loopWithScope', async () => {
    als.scope()
    als.setRemoveDelay(200)
    let count = 0
    /* eslint-disable no-await-in-loop */
    while (count < 300) {
      expect(als.size() < 20)
